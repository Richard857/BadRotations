...
-- Rotation Button
    RotationModes = {
        [1] = { mode = "Auto", value = 1 , overlay = "Automatic Rotation", tip = "基于范围内的目标数,在单目标和多目标自动切换输出方式.", highlight = 1, icon = br.player.spell.fanOfKnives },
        [2] = { mode = "Mult", value = 2 , overlay = "Multiple Target Rotation", tip = "使用多目标方式输出.", highlight = 0, icon = br.player.spell.fanOfKnives },
        [3] = { mode = "Sing", value = 3 , overlay = "Single Target Rotation", tip = "使用单目标方式输出.", highlight = 0, icon = br.player.spell.mutilate },
        [4] = { mode = "Off", value = 4 , overlay = "DPS Rotation Disabled", tip = "禁用DPS输出", highlight = 0, icon = br.player.spell.crimsonVial}
    };
    CreateButton("Rotation",1,0)
-- Cooldown Button
    CooldownModes = {
        [1] = { mode = "Auto", value = 1 , overlay = "Cooldowns Automated", tip = "自动使用大技能 - 只在BOSS.", highlight = 1, icon = br.player.spell.vendetta },
        [2] = { mode = "On", value = 1 , overlay = "Cooldowns Enabled", tip = "不管任何目标都使用大技能.", highlight = 0, icon = br.player.spell.vendetta },
        [3] = { mode = "Off", value = 3 , overlay = "Cooldowns Disabled", tip = "不使用大技能.", highlight = 0, icon = br.player.spell.vendetta }
    };
...
local function createOptions()
    ...
    local function rotationOptions()
        ...
        section = br.ui:createSection(br.ui.window.profile,  "一般")
            -- Opening Attack
            br.ui:createDropdown(section, "潜行攻击", {"锁喉", "偷袭"},  1, "|cffFFFFFF选择打破隐身的攻击技能")
            br.ui:createSpinnerWithout(section, "多目标Dot血量限制", 2, 0, 100, 1, "|cffFFFFFFHP X百万以上才会施放/刷新 割裂.")
            br.ui:createSpinnerWithout(section, "最大割裂目标数量", 3, 1, 100, 1, "|cffFFFFFFHP 最大割裂目标数量.")
            br.ui:createSpinnerWithout(section, "刀扇目标数量", 4, 0, 100, 1, "|cffFFFFFF多少敌人才使用刀扇")
            -- Poison
        	br.ui:createDropdown(section, "药膏", {"致命","致伤","苦痛"}, 1, "药膏")
        	br.ui:createDropdown(section, "非输出药膏", {"减速","吸血"}, 1, "非输出药膏")
            -- Poisoned Knife
        	br.ui:createCheckbox(section, "剧毒之刃")
            -- Stealth
            br.ui:createDropdown(section, "潜行", {"|cff00FF00总是", "|cffFFDD00PrePot", "|cffFF000020码"},  1, "使用潜行方式.")
            -- Shadowstep
            br.ui:createCheckbox(section, "暗影步")
            -- Opener
            ...
            -- Artifact
            br.ui:createDropdownWithout(section,"神器技能", {"|cff00FF00任何时候","|cffFFFF00冷却技能","|cffFF0000决不"}, 1, "|cffFFFFFF选择什么时候使用神器技能.")
        ...
        section = br.ui:createSection(br.ui.window.profile, "保命")
            ...
            -- Evasion
            br.ui:createSpinner(section, "闪避",  40,  0,  100,  5, "设置多少多少百分比血量使用 - 只在战斗!",  "|cffFFFFFF多少百分比血量使用")
            -- Cloak of Shadows
...
--------------
        ...
        dotHPLimit = getOptionValue("多目标Dot血量限制")
        maxrupture = getOptionValue("最大割裂目标数量")
        aoecount = getOptionValue("刀扇目标数量")
...
--------------------
    -- Action List - Extras
        ...
        -- Dummy Test
            if isChecked("DPS测试") then
                if GetObjectExists("target") then
                    if cTime >= (tonumber(getOptionValue("DPS测试"))*60) and isDummy() then
                        StopAttack()
                        ClearTarget()
        ...
        -- Poisoned Knife
            if isChecked("剧毒之刃") and not buff.stealth.exists() then
    ...
    -- Action List - Defensives
        local function actionList_Defensive()
            ...
            -- Heirloom Neck
                if isChecked("传家宝项链") and php <= getOptionValue("传家宝项链") and not inCombat then
    ...
    -- Action List - Cooldowns
        ...
        -- Potion
                ...
                if isChecked("爆发药水") and canUse(142117) then
        ...
        -- Racial
                ...
                if isChecked("种族技能") and ((debuff.vendetta.exists(units.dyn5) and (race == "Orc" or race == "Troll")) 
        ...
        -- Vanish
                ...
                if isChecked("消失") and not solo then
    ...
    -- Action List - PreCombat
        ...
        -- Apply Poison
            -- apply_poison
            if isChecked("药膏") then
                if br.timer:useTimer("药膏", 3.5) then
                    if getOptionValue("药膏") == 1 and not buff.deadlyPoison.exists() then
                        if cast.deadlyPoison() then return end
                    end
                    if getOptionValue("药膏") == 2 and not buff.woundPoison.exists() then
                        if cast.woundPoison() then return end
                    end
                    if getOptionValue("药膏") == 3 and not buff.agonizingPoison.exists() then
                        if cast.agonizingPoison() then return end
                    end
                end
            end
            if isChecked("非输出药膏") then
                if br.timer:useTimer("非输出药膏", 3.5) then
                    if (getOptionValue("非输出药膏") == 1 or not talent.leechingPoison) and not buff.cripplingPoison.exists() then
                        if cast.cripplingPoison() then return end
                    end
                    if getOptionValue("非输出药膏") == 2 and not buff.leechingPoison.exists() then
        ...
        -- Stealth
            -- stealth
           if isChecked("潜行") and not stealth and not inCombat and (not IsResting() or isDummy("target")) then
                if getOptionValue("潜行") == 1 then
                    if cast.stealth() then return end
                end 
                if #enemies.yards20 > 0 and getOptionValue("潜行") == 3 then
    ...
    -- Action List - Opener
        ...
        -- Shadowstep
            if isChecked("暗影步") and isValidUnit("target") and getDistance("target") > 8 and getDistance("target") < 25 then
        ...
        -- Start Attack
            ...
            elseif (UnitExists("target") and not isBoss("target")) or not isChecked("Opener") then
                if combo >= 2 and not debuff.rupture.exists(units.dyn5) and cTime < 10 and not artifact.urgeToKill and getOptionValue("潜行攻击") == 1 then
                    if cast.rupture("target") then opener = true; return end
                elseif combo >= 4 and not debuff.rupture.exists(units.dyn5) and getOptionValue("潜行攻击") == 1 then
                    if cast.rupture("target") then opener = true; return end
                elseif level >= 48 and not debuff.garrote.exists(units.dyn5) and cd.garrote <= 1 and getOptionValue("潜行攻击") == 1 then
                    if cast.garrote("target") then opener = true; return end
                elseif level >= 29 and getOptionValue("潜行攻击") == 2 then
...
----------------------------------
        -- Shadowstep
                if isChecked("暗影步") and getDistance("target") > 8 and getDistance("target") < 25 then
                    if cast.shadowstep("target") then return end
                end
                if opener == false then
                    if actionList_Opener() then return end
