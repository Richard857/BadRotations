...
-- Updates things Out of Combat like Talents, Gear, etc.
	function self.baseUpdateOOC()
		...
		self.baseGetEquip()
		if getOptionCheck("技能自动排队") and #self.queue ~= 0 then
...
---------------
 ...
 -- Call after Title
    function self.createBaseOptions()
        -- Base Wrap
        local section_base = br.ui:createSection(br.ui.window.profile, "基本选项")
        br.ui:createCheckbox(section_base, "自动进入战斗")
        br.ui:createCheckbox(section_base, "法术队列")
        br.ui:createDropdown(section_base, "暂停模式", br.dropOptions.Toggle, 2, "定义暂停运行的键.")
        br.ui:createCheckbox(section_base, "使用水晶")
        br.ui:createDropdown(section_base, "使用符文", {"|cff00FF00任何时候","|cffFF0000团队"}, 1, "任何时候或者只在团队中使用符文")
        br.ui:createCheckbox(section_base, "使用种族技能")
    	slsettings = br.ui:createCheckbox(section_base, "保存/加载 设置")
        br.ui:checkSectionState(section_base)
    end

 -- Get option modes
	function self.baseGetOptions()
		self.ignoreCombat             = isChecked("自动进入战斗")==true or false
		self.options.useCrystal       = isChecked("使用水晶")==true or false
		self.options.useEmpoweredRune = isChecked("使用符文",true)==true or false
		self.options.useRacial        = isChecked("使用种族技能")
		self.options.settings 		  = isChecked("保存/加载 设置")==true or false
	end
	...
	function self.useEmpoweredRune()
		if self.options.useEmpoweredRune and not UnitIsDeadOrGhost("player") then
			if self.level < 110 then
				...
				if getOptionValue("使用符文") == 2 and br.player.instance=="raid" then
					if getBuffRemain("player",self.augmentRune[self.primaryStat]) < 600 and not IsFlying() then
						if self.faction == "Alliance" and self.level < 110 then
							useItem(128482)
						else 
