...
-- Cooldown Button
    CooldownModes = {
        ...
        [2] = { mode = "On", value = 2 , overlay = "Cooldowns Enabled", tip = "不管任何目标都使用大技能.", highlight = 0, icon = br.player.spell.combustion},
        [3] = { mode = "Off", value = 3 , overlay = "Cooldowns Disabled", tip = "不使用大技能.", highlight = 0, icon = br.player.spell.combustion}
    };
    CreateButton("Cooldown",2,0)
-- Defensive Button
    DefensiveModes = {
        [1] = { mode = "On", value = 1 , overlay = "Defensive Enabled", tip = "使用保命技能.", highlight = 1, icon = br.player.spell.blazingBarrier},
        [2] = { mode = "Off", value = 2 , overlay = "Defensive Disabled", tip = "不使用保命技能.", highlight = 0, icon = br.player.spell.blazingBarrier}
    };
...
local function createOptions()
    ...
    -- Defensive Options
        ...
        -- Blazing Barrier
            br.ui:createCheckbox(section,"烈焰护体")
        br.ui:checkSectionState(section)
    -- Interrupt Options
...
--------------------
    ...
    -- Action List - Cooldowns
        ...
        -- Racial: Orc Blood Fury | Troll Berserking | Blood Elf Arcane Torrent
                ...
                if isChecked("种族") and (br.player.race == "Orc" or br.player.race == "Troll" or br.player.race == "Blood Elf") then
                    if castSpell("player",racial,false,false,false) then return end
                end
    ...
    -- Action List - PreCombat
        local function actionList_PreCombat()
            ...
            -- Mirror Image
                    if useCDs() and isChecked("镜像") then
...
--------------------------
    ...
    ---------------------------
        ...
        -- Blazing Barrier
                    if isChecked("烈焰护体") and not buff.blazingBarrier.exists() then
        ...
        -- Mirror Image
                    ...
                    if useCDs() and isChecked("镜像") and not buff.combustion.exists() then
                        if cast.mirrorImage() then return end
                    end
        -- Rune of Power
