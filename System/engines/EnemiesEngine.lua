		...
		for k, v in pairs(br.enemy) do
			...
			if isChecked("只有敌人") == false or (getOptionCheck("只有敌人") and UnitReaction(thisUnit.unit,"player")) == 2 then
				if GetUnitExists(thisUnit.unit) and ObjectID(thisUnit.unit) ~= 103679 and thisUnit.coeficient ~= nil and getLineOfSight("player", thisUnit.unit) and not UnitIsTrivial(thisUnit.unit) and UnitCreatureType(thisUnit.unit) ~= "Critter" then
					if (not getOptionCheck("安全攻击检测") or thisUnit.safe) and not thisUnit.isCC
...
-- 易爆助手
function fOrb()
	local t, o, id="target", nil, 120651
		if getOptionCheck("易爆助手") and not tOrb and(not UnitExists(t)or ObjectID(t)~=id)then 
		   for i=1, ObjectCount()do o=ObjectWithIndex(i)if ObjectID(o)==id then 
		        TargetUnit(o)
		        end
		    end 
	    end 
    end 
		AddTimerCallback(.25, fOrb)
-- function dynamicTarget(range,facing)
-- 	if getOptionCheck("动态定位") then
-- 		local bestUnitCoef = 0
-- 		local bestUnit = "target"
...
function getUnitCoeficient(unit,distance,threat,burnValue,shieldValue)
	...
	if GetObjectExists(unit) then
		...
		if distance < 40 then
			...
			if getOptionCheck("智能目标") == true then
				if getOptionValue("智能目标") == 1 then -- Highest
					-- if highest is selected
					coef = unitHP
				elseif getOptionValue("智能目标") == 3 then -- abs Highest
					coef = UnitHealth(unit)
				elseif getOptionValue("智能目标") == 4 then -- Furthest
					coef = 100 - distance
				elseif getOptionValue("智能目标") == 5 then -- Nearest
					coef = distance
				else 										   -- Lowest
			...
			if getOptionCheck("优先攻击骷髅头目标") and GetRaidTargetIndex(unit) == 8 then
				coef = coef + 50
			end
			-- if threat is checked, add 100 points of prio if we lost aggro on that target
			if getOptionCheck("坦克仇恨") == true then
...
function isSafeToAttack(unit)
	if getOptionCheck("安全攻击检测") == true then
		-- check if unit is valid
