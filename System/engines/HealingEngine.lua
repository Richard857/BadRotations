...
if not metaTable1 then
	...
	function HealCheck(tar)
		if ((UnitIsVisible(tar)
			...
			and UnitInPhase(tar))
			or novaEngineTables.SpecialHealUnitList[tonumber(select(2,getGUID(tar)))] ~= nil	or (getOptionCheck("治疗宠物") == true and UnitIsOtherPlayersPet(tar) or UnitGUID(tar) == UnitGUID("pet")))
	...
	function memberSetup:new(unit)
		...
		function o:Dispel()
			for i = 1, #novaEngineTables.DispelID do
				if UnitDebuff(o.unit,GetSpellInfo(novaEngineTables.DispelID[i].id)) ~= nil and novaEngineTables.DispelID[i].id ~= nil then
                    ...
                    and (isChecked("驱散延迟") and
                            (getDebuffDuration(o.unit, novaEngineTables.DispelID[i].id) - getDebuffRemain(o.unit, novaEngineTables.DispelID[i].id)) > (getDebuffDuration(o.unit, novaEngineTables.DispelID[i].id) * (math.random(getValue("驱散延迟")-2, getValue("驱散延迟")+2)/100) ))then -- Dispel Delay
		...
		function o:CalcHP()
			...
			-- incoming heals
			local incomingheals
			if getOptionCheck("预计治疗") == true and UnitGetIncomingHeals(o.unit,"player") ~= nil then
			...
			-- absorbs
			local nAbsorbs
			if getOptionCheck("忽略吸收") ~= true then
			...
			if getOptionCheck("黑名单") == true and br.data.blackList ~= nil then
				for i = 1, #br.data.blackList do
					if o.guid == br.data.blackList[i].guid then
						PercentWithIncoming,ActualWithIncoming,nAbsorbs = PercentWithIncoming + getValue("黑名单"),ActualWithIncoming + getValue("黑名单"),nAbsorbs + getValue("黑名单")
						break
					end
				end
			end
	...
	function SetupTables() -- Creating the cache (we use this to check if some1 is already in the table)
		...
		function br.friend:Update()
			...
			if br.friendTableTimer == nil or br.friendTableTimer <= GetTime() - refreshTimer then
				...
				local selectedMode,SpecialTargets = getOptionValue("特殊治疗"), {}
				if getOptionCheck("特殊治疗") == true then
				...
				if getOptionCheck("优先治疗") == true then
