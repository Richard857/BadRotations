...
function canRun()
	if getOptionCheck("Pause") ~= 1 then
		if getOptionCheck("开始/停止 BR") and isAlive("player") then
			if SpellIsTargeting()
	...
	if GetObjectExists(Unit) then
		if UnitIsEnemy("player", Unit) then
			...
			if not UnitIsDeadOrGhost(Unit) and GetUnitIsVisible(Unit) then
				...
				if getOptionCheck("预计治疗") == true and UnitGetIncomingHeals(Unit,"player") ~= nil then
...
function pause(skipCastingCheck)
	...
	-- 	Print("Right Clicked")
	-- end
	if SpecificToggle("暂停模式") == nil or getValue("暂停模式") == 6 then
		pausekey = IsLeftAltKeyDown()
	else
		pausekey = SpecificToggle("暂停模式")
	end
	-- DPS Testing
	...
	-- Pause Hold/Auto
	if (pausekey and GetCurrentKeyBoardFocus() == nil and isChecked("暂停模式"))
		or profileStop
...
function spellDebug(Message)
	if imDebugging == true and getOptionCheck("保命模式") then
		ChatOverlay(Message)
	end
end
