...
-- Set Main Button
	...
	mainButton:SetScript("OnClick", function()
		if br.data.settings[br.selectedSpec].toggles['Power'] ~= 0 then
			...
			mainButton:SetNormalTexture(backIconOff)
			-- on/off switch
			if br.data.settings[br.selectedSpec]["开始/停止 BRCheck"] ~= 0 then
				br.data.settings[br.selectedSpec]["开始/停止 BRCheck"] = 0
			end
			GameTooltip:SetText("|cff00FF00启用 |cffFF0000BadRotations \n|cffFFDD11按住左Alt并滚动鼠标滚轮可以调整大小.", 225/255, 225/255, 225/255)
			mainButtonFrame.texture:SetTexture(genericIconOff)
		else
			br.data.settings[br.selectedSpec].toggles['Power'] = 1
			-- on/off switch
			if br.data.settings[br.selectedSpec]["开始/停止 BRCheck"] ~= 1 then
				br.data.settings[br.selectedSpec]["开始/停止 BRCheck"] = 1
			end
			GameTooltip:SetText("|cffFF0000禁用 BadRotations \n|cffFFDD11按住左Alt并滚动鼠标滚轮可以调整大小.", 225/255, 225/255, 225/255)
			mainButton:SetNormalTexture(backIconOn)
