...
function lootManager:getLoot()
	...
	if lM:emptySlots() ~= 0 then
		if UnitCastingInfo("player") == nil and UnitChannelInfo("player") == nil and DontMoveStartTime and GetTime() - DontMoveStartTime > 0 then
			...
			if br.timer:useTimer("getLoot", getOptionValue("自动拾取")) and lootFound then
...
function lootManager:findLoot()
	if br.timer:useTimer("findLoot", getOptionValue("自动拾取")) then
		lM:debug("Find Unit")
...
function autoLoot()
	...
	if getOptionCheck("远程拾取器") and not moving and (not LootARangTime or GetTime() - LootARangTime > getOptionValue("远程拾取器")) then
	    if getOptionValue("远程拾取器 用法") == 1 and not isInCombat("player") and not lootFound and getOptionCheck("远程拾取器") and PlayerHasToy(60854) and GetItemCooldown(60854)==0 then
	        useItem(60854)
			LootARangTime = GetTime()
		end
	    if getOptionValue("远程拾取器 用法") == 2 and not lootFound and getOptionCheck("远程拾取器") and PlayerHasToy(60854) and GetItemCooldown(60854)==0 then
	        useItem(60854)
			LootARangTime = GetTime()
		end		
	end	
	if getOptionCheck("老妈的剥皮小刀") and not moving and (not MothersSkinningKnifeTime or GetTime() - MothersSkinningKnifeTime > getOptionValue("远程拾取器")) then
	    if getOptionValue("老妈的剥皮小刀 用法") == 1 and not isInCombat("player") and not lootFound and getOptionCheck("老妈的剥皮小刀") and PlayerHasToy(130102) and GetItemCooldown(130102)==0 then
	        useItem(130102)
			MothersSkinningKnifeTime = GetTime()
		end
	    if getOptionValue("老妈的剥皮小刀 用法") == 2 and not lootFound and getOptionCheck("老妈的剥皮小刀") and PlayerHasToy(130102) and GetItemCooldown(130102)==0 then
	        useItem(130102)
			MothersSkinningKnifeTime = GetTime()
		end
	end	
