...
function EnemyEngine(_, time)
	...
	if getOptionValue("执行率") ~= nil and getOptionValue("执行率") * 2 > 0.5 then updateRate = getOptionValue("执行率") * 2
		else updateRate = 0.5
	end
	--print(updateRate)
...
function BadRotationsUpdate(self)
	...
	if br.updateInProgress ~= true then
		...
		if getOptionValue("执行率") == nil then updateRate = 0.1 else updateRate = getOptionValue("执行率") end
		if self.lastUpdateTime and (tempTime - self.lastUpdateTime) > updateRate then --0.1 then
			self.lastUpdateTime = tempTime
			-- Check for Unlocker
			if FireHack == nil then
			 	br.ui:closeWindow("all")
				if getOptionCheck("开始/停止 BR") then
					ChatOverlay("FireHack未加载.")
					if isChecked("通知未解锁") and br.timer:useTimer("notLoaded", getOptionValue("通知未解锁")) then
						Print("|cffFFFFFF无法启动... |cffFF1100Firehack |cffFFFFFF未加载. 请注入Firehack.")
					end
				end
				return false
			else
				...
				-- Queue Casting
					if isChecked("技能自动排队") and not UnitChannelInfo("player") then
						-- Catch for spells not registering on Combat log
					    if castQueue() then return end
					end

				-- LoS Line Draw
					if isChecked("治疗者的视线指标") then
						inLoSHealer()
					end
